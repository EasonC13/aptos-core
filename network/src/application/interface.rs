...
impl<Message: NetworkMessageTrait + Clone> NetworkClient<Message> {
    ...
    /// preferred protocol cache so that it can reloaded.
    /// This is required to support peers who change their
    /// preferred/supported protocols and reconnect.
    pub fn spawn_preferred_protocol_cache_invalidator(
        ...
        // Create the cache invalidator
        let preferred_protocol_for_peer_cache = self.preferred_protocol_for_peer_cache.clone();
        let cache_invalidator = async move {
            // Create a ticker for the invalidation interval
            let duration = Duration::from_millis(cache_invalidation_frequency_secs);
        ...
        runtime.spawn(cache_invalidator)
    }
    ...
    fn get_preferred_protocol_for_peer(
        ...
        let all_preferred_protocols = match message_type {
        ...
        let preferred_protocol =
            self.calculate_preferred_protocol_for_peer(peer, all_preferred_protocols)?;
        let _ = self
            .preferred_protocol_for_peer_cache
            .write()
